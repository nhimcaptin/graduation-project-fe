    ...
    await Promise.all(
      comments.map(async (comment) => {
        ...
        commentsWithDoctor.push({
          ...
          date: formattedDate,
          image: user ? user.image : "Không xác định",
        });
      })
    );
...
export const deleteComment = async (req, res, next) => {
  //delete commnet màn hình end user
  try {
  ...
  try {
    ...
    export const getComments = async (req, res, next) => {
      try {
        ...
        const query = Comment.find(_filter);
        const users = await query
          .populate("comment")
          .skip((page - 1) * pageSize)
          .limit(pageSize)
          .sort(Sorts);
        const totalUsers = await Comment.countDocuments(Comment.find(_filter));
