...
const SunEditorShare = (props: SunEditorModel) => {
  ...
  const onImageUploadBefore = (files: Array<File>, info: object, uploadHandler: Function) => {
    if (files.length) {
      ...
      uploadFileService
        .uploadFileResources(bodyFormData)
        .then((resUpload: any) => {
          if (resUpload) {
            let result = [
              {
                url: resUpload.downloadURL ? resUpload.downloadURL : "",
                name: files[0].name || "",
                size: files[0].size || 0,
              },
            ];
            ...
            uploadHandler(MESSAGE_ERROR.suneditorUploadFile);
          }
        })
        .catch((err: any) => {
          uploadHandler(handleErrorMessage(err));
        });
    }
    return undefined;
  };
