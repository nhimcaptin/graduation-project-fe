    ...
    return new Promise((resolve, reject) => {
      ...
      if (filter) {
        ...
        for (let propertyName in filter) {
          if (filter[propertyName] !== undefined && filter[propertyName] !== null && filter[propertyName] !== "") {
            if (propertyName === "fullName") {
              ...
              for (let dateRange in filter[propertyName]) {
                param += `${
                  ...
                  !!filter[propertyName][dateRange].end ? `${dateRange}<=${filter[propertyName][dateRange].end},` : ""
                }`;
              }
              ...
              for (let equal in filter[propertyName]) {
                if (filter[propertyName][equal]) {
                  param += `${equal}!=${encodeURIComponent(convertSearchString(filter[propertyName][equal]))},`;
                }
              }
              ...
              param += `${propertyName}@=${encodeURIComponent(convertSearchString(filter[propertyName]))},`;
            }
          }
        }
        apiUrl += "&" + param;
