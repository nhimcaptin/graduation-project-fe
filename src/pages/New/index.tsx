...
const New = (props: any) => {
  ...
  const handleDelete = () => {
    ...
    openConfirmModal({
      isOpen: true,
      title: "Xóa",
      message: MESSAGES_CONFIRM.DeleteNew,
      cancelBtnLabel: "Hủy",
      okBtnLabel: "Xóa",
  ...
  const onDelete = async () => {
    ...
    try {
      await apiService.delete(`${URL_PATHS.DELETE_NEW}/${selectedItem?._id}`);
      setToastInformation({
  ...
  const getData = async (props: any) => {
    ...
    try {
      const data: any = await apiService.getFilter(URL_PATHS.GET_LIST_NEWS, params, filters);
    //   console.log(data);
      const _item = (data?.getNews || []).map((x: any) => {
        return {
          ...x,
  ...
  const getDetail = async (id: string | number) => {
    setLoadingScreen(true);
    try {
      const data: any = await apiService.getFilter(`${URL_PATHS.GET_DETAIL_NEWS}/${id || selectedItem?._id}`);
    //   console.log(data);
  ...
  return (
    <Page className={styles.root} title="Tin tức" isActive>
