...
import AddEditKnowledge from "./components/AddEditKnowledge";
...
const New = (props: any) => {
  ...
  const handleDelete = () => {
    ...
    openConfirmModal({
      isOpen: true,
      title: "Xóa",
      message: MESSAGES_CONFIRM.DeleteKnowledge,
      cancelBtnLabel: "Hủy",
      okBtnLabel: "Xóa",
  ...
  const onDelete = async () => {
    ...
    try {
      await apiService.delete(`${URL_PATHS.DELETE_KNOWLEDGE}/${selectedItem?._id}`);
      setToastInformation({
        status: STATUS_TOAST.SUCCESS,
        message: MESSAGE_SUCCESS.DELETE_KNOWLEDGE,
      });
      getData && getData({});
    } catch (error: any) {
      setToastInformation({
  ...
  const getData = async (props: any) => {
    ...
    try {
      const data: any = await apiService.getFilter(URL_PATHS.GET_LIST_KNOWLEDGE, params, filters);
      // console.log(data);
      const _item = (data?.getDentalKnowledge || []).map((x: any) => {
        return {
          ...x,
  ...
  const getDetail = async (id: string | number) => {
    setLoadingScreen(true);
    try {
      const data: any = await apiService.getFilter(`${URL_PATHS.DETAIL_KNOWLEDGE}/${id || selectedItem?._id}`);
    //   console.log(data);
      setMainServiceDetail(data?.data);
      setIsOpenModal(true);
    } catch (error: any) {
      setToastInformation({
        status: STATUS_TOAST.ERROR,
  ...
  return (
    <Page className={styles.root} title="Kiến thức nha khoa" isActive>
                    ...
                    <Controller
                      ...
                      render={({ field: { onChange, onBlur, value, ref, name } }) => (
                        <TextFieldCustom
                          ...
                          value={value}
                          onChange={onChange}
                          placeholder="Nhập tên kiến thức nha khoa"
                          type="text"
                        />
                      )}
                    />
